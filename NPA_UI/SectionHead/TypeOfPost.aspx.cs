...
public partial class SectionHead_TypeOfPost : System.Web.UI.Page
{
    ...
    protected void btn_Save_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (validate())
            {
                objBE.PostTypeName = txtPosttypename.Text.Trim();
                objBE.UserID = user;
                objBE.Action = "I";
    ...
    protected void BindGrid()
    {
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                GVPostType.DataSource = dt;
                GVPostType.DataBind();
            }
            else
            {
                GVPostType.DataSource = null;
                GVPostType.DataBind();
    ...
    protected void GVPostType_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            if (e.CommandName == "Edt")
            {
                ...
                txtPosttypename.Text = ((Label)(grdDetails.FindControl("lblpostType"))).Text;
                Label1.Text = ((Label)(grdDetails.FindControl("lblpostTypecode"))).Text;
            ...
            {
                GridViewRow grdDetails = (GridViewRow)((Control)e.CommandSource).NamingContainer;
                objBE.PostTypeId = ((Label)(grdDetails.FindControl("lblpostTypecode"))).Text;
                objBE.Action = "D";
                dt = ObjDL.TypeOfPostIUDR(objBE, con);
                if (dt.Rows.Count > 0)
                {
    ...
    protected void btn_Update_Click(object sender, EventArgs e)
    {
        ...
        {
            objBE = new Master_BE();
            objBE.PostTypeName = txtPosttypename.Text;
            objBE.PostTypeId = Label1.Text;
            objBE.Action = "U";
            dt = ObjDL.TypeOfPostIUDR(objBE, con);
            if (dt.Rows.Count > 0)
            {
